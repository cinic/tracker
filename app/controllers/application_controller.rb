class ApplicationController < ActionController::Base
  ...
  def layout
    if self.class.name.split('::').first == 'Admin'
      ...
      'frontend'
    elsif controller_name == 'devices' || ( controller_name == 'users' && action_name == 'show' )
      'general'
    else
      'application'
    end
  ...
  def configure_permitted_parameters
    devise_parameter_sanitizer.for(:sign_up) { |u| u.permit(:role, :name, :company, :address, :email, :password) }
  end
